...
```{r}
#––– 0) LOAD + PREP DATA –––
gene_expression_stage_34 <- read_csv("/Users/usuario/OneDrive/Desktop/Thesis/personal_files/stages/gene_expression_stage_34.csv") %>%
...
coef_enet <- read_csv("/Users/usuario/OneDrive/Desktop/Thesis/personal_files/stages/coef_stage12/OS_coefficients_Elastic_Net.csv")
...
```{r}
# Genes selected by Adaptive Ridge on breast cancer (prior)
prior_genes <- coef_enet$Variable

# Genes selected by the current LASSO model (non-zero coefficients)
selected_genes <- names(selected_betas)

# Intersect to find common genes
common_genes <- intersect(prior_genes, selected_genes)
