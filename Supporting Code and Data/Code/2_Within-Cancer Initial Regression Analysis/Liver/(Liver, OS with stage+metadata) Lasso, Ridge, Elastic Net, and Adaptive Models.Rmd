...
This is gene expression + the metadata variables
```{r}
#load datasets
#gene_expression_breast <- read_csv("/Users/inigo/Downloads/R/gene_expression_only_breast_final.csv")
liver <- read_csv("/Users/inigo/Downloads/liver_combined_final.csv")
```

```{r}
# we drop the column id from the regression
liver <- liver[ , !(names(liver) %in% c("id",'OS.time','T_grouped_TX','is_alive'))]
...
liver <- as.data.frame(scale(liver))
```
...
set.seed(236)
X <- as.matrix(liver[ , names(liver) != "OS"])
n <- nrow(X)
...
setwd("~/Desktop/BSE/Term 3/Thesis/master_thesis_git/liver cancer")

# Save each model’s coefficients to its own CSV file
write.csv(coef_df_non_zero,        file = "combined_with_metadata_coefficients_Lasso.csv",             row.names = FALSE)
write.csv(coef_df_adapt_non_zero,  file = "combined_with_metadata_coefficients_Adaptive_Lasso.csv",    row.names = FALSE)
write.csv(coef_df_enet_non_zero,   file = "combined_with_metadata_coefficients_Elastic_Net.csv",       row.names = FALSE)
write.csv(coef_adapt_enet_non_zero,file = "combined_with_metadata_coefficients_Adaptive_from_EN.csv",  row.names = FALSE)
...
set.seed(236)
cv_ridge <- cv.glmnet(X_train, y_train, family = "binomial", alpha = 0, type.measure = "auc", nfolds = 10, nlambda = 200)
...
cat("Ridge selected", nrow(coef_df_ridge_non_zero), "genes with non-zero coefficients.\n")
print(coef_df_ridge_non_zero)

# Predict probabilities on the test set
fitted_probs_ridge <- as.vector(predict(cv_ridge, newx = X_test, s = lambda_ridge, type = "response"))

# ROC and AUC
roc_ridge <- roc(y_test, fitted_probs_ridge, positive = "1", quiet = TRUE)
plot(roc_ridge, legacy.axes = TRUE, print.thres = TRUE, print.thres.col = "darkblue", main = "ROC - Ridge (Test Set)")
cat("Ridge Test AUC:", round(auc(roc_ridge), 3), "\n")

# Best threshold
threshold_ridge <- coords(roc_ridge, "best", ret = "threshold")[[1]]
cat("Automatically selected threshold:", round(threshold_ridge, 4), "\n")

# Predict classes
pred_class_ridge <- ifelse(fitted_probs_ridge >= threshold_ridge, 1, 0)

# Confusion matrix
conf_matrix_ridge <- table(Predicted = pred_class_ridge, Actual = y_test)
cat("\nConfusion Matrix:\n")
print(conf_matrix_ridge)

# Row-wise proportions
cat("\nRow-wise proportions:\n")
print(round(prop.table(conf_matrix_ridge, 1), 3))

# Metrics
TP <- sum(pred_class_ridge == 1 & y_test == 1)
TN <- sum(pred_class_ridge == 0 & y_test == 0)
FP <- sum(pred_class_ridge == 1 & y_test == 0)
FN <- sum(pred_class_ridge == 0 & y_test == 1)

precision <- TP / (TP + FP)
recall <- TP / (TP + FN)
f1 <- 2 * precision * recall / (precision + recall)
specificity <- ifelse((TN + FP) > 0, TN / (TN + FP), NA)
accuracy <- mean(pred_class_ridge == y_test)
...
coef_df_ridge_non_zero <- subset(coef_df_ridge, value != 0 & Variable != "(Intercept)")
cat("Ridge selected", nrow(coef_df_ridge_non_zero), "genes.\n")
print(coef_df_ridge_non_zero)

# Save to CSV
write.csv(coef_df_ridge_non_zero, file = "ridge_selected_genes.csv", row.names = FALSE)
cat("Ridge results saved to ridge_selected_genes.csv\n")
```
...
```{r}
# ADAPTIVE RIDGE FOR Y_OS

# Use Ridge non-zero coefficients (excluding intercept) as base
initial_coefs <- coef_df_ridge_non_zero$value
non_zero_vars <- coef_df_ridge_non_zero$Variable

# Select corresponding columns in X_train and X_test
...
lambda_adaptive <- cv_adaptive_ridge$lambda.min

# Extract non-zero coefficients
coef_adapt <- coef(cv_adaptive_ridge, s = lambda_adaptive)
coef_df_adapt <- as.data.frame(as.matrix(coef_adapt))
colnames(coef_df_adapt)[1] <- "value"
coef_df_adapt$Variable <- rownames(coef_df_adapt)
...
fitted_probs_adapt <- as.vector(predict(cv_adaptive_ridge, newx = X_test_selected, s = lambda_adaptive, type = "response"))

# ROC and AUC
roc_adapt <- roc(y_test, fitted_probs_adapt, positive = "1", quiet = TRUE)
plot(roc_adapt, legacy.axes = TRUE, print.thres = TRUE, print.thres.col = "purple", main = "ROC - Adaptive Ridge (Test Set)")
...
# Predict classes
pred_class_adapt <- ifelse(fitted_probs_adapt >= threshold_adapt, 1, 0)
...
# Save to CSV
write.csv(coef_df_adapt_non_zero, file = "adaptive_ridge_selected_genes.csv", row.names = FALSE)
cat("Adaptive Ridge results saved to adaptive_ridge_selected_genes.csv\n")
```
