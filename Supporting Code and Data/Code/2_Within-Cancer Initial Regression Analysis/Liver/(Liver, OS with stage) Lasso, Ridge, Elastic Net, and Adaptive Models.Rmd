...
gene_expression_liver <- gene_expression_liver[ , !(names(gene_expression_liver) %in% c("id"))]

```

```{r}
# Extract target variable
y_OS <- gene_expression_liver[["OS"]]

```

```{r}
#Standarize the gene expression
gene_expression_liver <- as.data.frame(scale(gene_expression_liver))
```
...
set.seed(236)
X <- as.matrix(gene_expression_liver[ , names(gene_expression_liver) != "OS"])
n <- nrow(X)
...
# Identify model with highest AUC
best_model_idx <- which.max(all_performance$AUC)
best_model_name <- all_performance$Model[best_model_idx]
cat("\nBest model based on AUC:", best_model_name, "\n")

# Select the corresponding coefficient data frame
best_coef_df <- switch(
  best_model_name,
  "Lasso"            = coef_df_non_zero,
  "Adaptive Lasso"   = coef_df_adapt_non_zero,
  "Elastic Net"      = coef_df_enet_non_zero,
  "Adaptive from EN" = coef_adapt_enet_non_zero
)

setwd("~/Desktop/BSE/Term 3/Thesis/master_thesis_git/liver cancer")

# Save to CSV
write.csv(best_coef_df, file = paste0("combined_best_model_coefficients_", gsub(" ", "_", best_model_name), ".csv"), row.names = FALSE)
...
```{r}
setwd("C:/Users/usuario/OneDrive/Desktop/Thesis/personal_files/liverselected")
...
# Select corresponding columns in training and test sets
...
# Fit Adaptive Ridge with adaptive weights
set.seed(236)
...
cat("Optimal Lambda (Adaptive Ridge):", lambda_adaptive, "\n")

# Extract Adaptive Ridge coefficients
...
coef_df_adapt_non_zero <- subset(coef_df_adapt, value != 0 & Variable != "(Intercept)")
cat("Adaptive Ridge selected", nrow(coef_df_adapt_non_zero), "variables.\n")
print(coef_df_adapt_non_zero)

# Predict probabilities on the test set
fitted_probs_adapt <- as.vector(predict(cv_adaptive_ridge, newx = X_test_selected, s = lambda_adaptive, type = "response"))

# ROC curve and AUC for Adaptive Ridge
roc_adaptive <- roc(y_test, fitted_probs_adapt, positive = "1", quiet = TRUE)
plot(roc_adaptive, legacy.axes = TRUE, print.thres = TRUE, 
     print.thres.col = "blue", main = "ROC - Adaptive Ridge (Test Set)")

cat("Adaptive Ridge Test AUC:", round(auc(roc_adaptive), 3), "\n")

# Save Adaptive Ridge coefficients to CSV in the working directory
write.csv(coef_df_adapt_non_zero, file = "combined_coefficients_AdaptiveRidge.csv", row.names = FALSE)
