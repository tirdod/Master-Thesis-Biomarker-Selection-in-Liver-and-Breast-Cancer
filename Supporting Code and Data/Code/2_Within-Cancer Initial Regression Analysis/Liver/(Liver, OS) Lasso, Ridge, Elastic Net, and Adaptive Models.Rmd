...
```{r}
#load datasets
#gene_expression_ <- read_csv("/Users/inigo/Downloads/R/gene_expression_only_t_final.csv")
#gene_expression_liver <- read_csv("~/Desktop/BSE/Term #3/Thesis/master_thesis_git/liver cancer/gene_expression_only_liver_final.csv"
#)
...
gene_expression_liver <- gene_expression_liver[ , !(names(gene_expression_liver) %in% c("Is_stage_34", "id"))]
...
# Save to CSV
write.csv(best_coef_df, file = paste0("OS_best_model_coefficients_", gsub(" ", "_", best_model_name), ".csv"), row.names = FALSE)
...
# Use Ridge non-zero coefficients (excluding intercept) as starting point
...
write.csv(
  ...
  file = "OS_coefficients_AdaptiveRidge.csv",
  row.names = FALSE
)
