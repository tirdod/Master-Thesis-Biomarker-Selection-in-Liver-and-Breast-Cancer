...
```{r}
# we drop the column OS from the regression
gene_expression_liver <- gene_expression_liver[ , !(names(gene_expression_liver) %in% c("OS", "id"))]

```

```{r}
#Get target variables
y_stage <- gene_expression_liver[["Is_stage_34" ]]
...
set.seed(236)
X <- as.matrix(gene_expression_liver[ , names(gene_expression_liver) != "Is_stage_34"])
n <- nrow(X)
y<-factor(y_stage,levels = c(0, 1))
...
# Function to summarize performance with covariate count
...
# Collect all performance metrics into a single table
...
# Save to CSV
write.csv(best_coef_df, file = paste0("stage_best_model_coefficients_", gsub(" ", "_", best_model_name), ".csv"), row.names = FALSE)
...
write.csv(coef_df_ridge_non_zero, file = "isstage_coefficients_Ridge.csv", row.names = FALSE)
...
# Select columns corresponding to non-zero Ridge variables
...
roc_adapt <- roc(y_test, fitted_probs_adapt, positive = "1", quiet = TRUE)
plot(roc_adapt, legacy.axes = TRUE, print.thres = TRUE, 
     print.thres.col = "blue", main = "ROC - Adaptive Ridge (Test Set)")

cat("Adaptive Ridge Test AUC:", round(auc(roc_adapt), 3), "\n")

# Save Adaptive Ridge coefficients to CSV
write.csv(coef_df_adapt_non_zero, file = "isstage_coefficients_AdaptiveRidge.csv", row.names = FALSE)
