...
#gene_expression_breast <- read_csv("C:/Users/usuario/OneDrive/Desktop/Thesis/personal_files/gene_expression_only_breast_final.csv")
```

```{r}
# we drop the column OS from the regression
gene_expression_breast <- gene_expression_breast[ , !(names(gene_expression_breast) %in% c("OS", "id"))]

```

```{r}
#Get target variables
y_stage <- gene_expression_breast[["Is_stage_34" ]]
...
set.seed(236)
X <- as.matrix(gene_expression_breast[ , names(gene_expression_breast) != "Is_stage_34"])
n <- nrow(X)
...
# Save to CSV
write.csv(
  coef_df_ridge_non_zero,
  "/Users/inigo/Downloads/selected_genes_ridge_isstage.csv",
  row.names = FALSE
)
...
```{r}
# Prepare Adaptive Ridge using previous Ridge coefficients
set.seed(236)

# Get initial coefficients (excluding intercept)
initial_coefs <- coef_df_ridge_non_zero$value
non_zero_vars <- coef_df_ridge_non_zero$Variable

# Select columns from X_train and X_test that match non-zero variables
X_train_selected <- X_train[, non_zero_vars, drop = FALSE]
X_test_selected  <- X_test[, non_zero_vars, drop = FALSE]

# Define adaptive weights (inverse of absolute values of Ridge coefficients)
...
selected_genes_enet <- coef_df_enet_non_zero[, c("Variable", "value")]


# Save to CSV
write.csv(
  selected_genes_enet,
  "C:/Users/usuario/OneDrive/Desktop/Thesis/personal_files/breastselected/selected_genes_elasticnet_is_stage.csv",
  row.names = FALSE
)
